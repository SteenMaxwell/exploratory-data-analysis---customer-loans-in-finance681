'''
Imports necessary for loading YAML configuration files, handling data frames
with pandas, and creating database connections using SQLAlchemy.
'''
...
def loading_credentials():
    '''
    This function loads the credentials from a YAML file
    for the AWS RDS so the dataset can be accessed locally.
...
class RDSDatabaseConnector:
    '''
    Used to ultimately access an AWS RDS database and save the 'loan_payments' 
    table to a CSV file so it can be accessed locally.
    ...
    def extract_data(self):
        '''
        This method extracts the dataset 'loan_payments' from the AWS RDS database 
        ...
        extracted_df = pd.read_sql_table('loan_payments', self.engine)
        return extracted_df
